      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-6 max-w-7xl mx-auto">
          {certs.map((cert, index) => (
            ...
            viewport={{ once: true, margin: "-50px" }}
              transition={{ duration: 0.6, ease: "easeOut" }}
              className="p-8 rounded-3xl bg-white/[0.03]  border border-white/5 hover:border-cyan/30 hover:shadow-[0_0_30px_rgba(21,209,233,0.15)] hover:-translate-y-2 transition duration-75 ease-out group relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-cyan/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-75 ease-out z-0 pointer-events-none"></div>
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-12 h-12 rounded-full bg-cyan/10 flex items-center justify-center text-cyan group-hover:scale-110 group-hover:bg-cyan group-hover:text-black transition duration-75 ease-out shadow-[0_0_15px_rgba(21,209,233,0.2)] group-hover:shadow-[0_0_20px_rgba(21,209,233,0.5)]">
                    <cert.Icon size={24} />
                  </div>
                  <div>
                    <h4 className="text-xl font-bold text-white group-hover:text-cyan transition-colors duration-75 ease-out">{cert.title}</h4>
