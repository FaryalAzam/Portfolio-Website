      ...
      {category.skills.map((skill, index) => {
        ...
        return (
          <div
            ...
            className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl transition duration-75 ease-out cursor-pointer ${
              isBig 
          ...
          >
            <div className={`text-3xl transition-transform duration-75 ease-out ${isBig ? 'scale-110' : ''}`}>
              {skill.icon}
            </div>
            <span className={`text-xs font-medium transition-colors duration-75 ease-out ${isBig || isSmall ? 'text-white' : 'text-lightGrey'}`}>
              {skill.name}
            </span>
          </div>
        );
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {categories.map((category, idx) => (
            ...
            viewport={{ once: true, margin: "-50px" }}
              transition={{ duration: 0.6, ease: "easeOut" }}
              className="p-8 rounded-3xl bg-white/[0.02]  border border-white/5 hover:border-cyan/30 hover:bg-white/[0.04] transition duration-75 ease-out hover:shadow-[0_0_30px_rgba(21,209,233,0.1)] group relative overflow-hidden"
            >
