          ...
          <motion.div variants={itemVariants} className="flex flex-wrap justify-center gap-3">
            {categories.map((cat, index) => (
              <button
                ...
                className="relative px-5 py-2 rounded-full text-sm font-medium transition--colors duration-75 ease-out ease-out outline-none"
              >
                {activeCategory === cat && (
                  <motion.div 
                ...
                <span className={`relative z-10 transition--colors duration-75 ease-out ease-out ${activeCategory === cat ? 'text-black font-semibold' : 'text-lightGrey hover:text-white'}`}>
                  {cat}
                </span>
              </button>
            ))}
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, index) => (
              <motion.div
                ...
                className={`group relative rounded-3xl p-[1px] bg-white/[0.05] hover:bg-gradient-to-br hover:from-cyan hover:via-orange hover:to-cyan transition- duration-75 ease-out ease-out shadow-lg hover:shadow-[0_0_40px_rgba(21,209,233,0.3)]  overflow-hidden ${
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-br from-cyan/10 to-transparent opacity-0 group-hover:opacity-100 transition--opacity duration-75 ease-out ease-out z-0 pointer-events-none"></div>
                <div className="flex flex-col bg-black/80  rounded-[23px] overflow-hidden h-full relative z-10">
                  ...
                  <div className="relative w-full aspect-video bg-black/60 overflow-hidden p-2">
                    <img 
                      src={project.image} 
                      alt={project.name} 
                      className="w-full h-full object-contain object-center transition--transform duration-75 ease-out ease-out group-hover:scale-110 rounded-2xl" 
                    />
                    
                    {/* Hover Overlay with Icons */}
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition--opacity duration-75 ease-out ease-out flex items-center justify-center gap-4 backdrop-blur-[2px]">
                      <a 
                        href={project.link} 
                        target="_blank" 
                        rel="noreferrer" 
                        className="w-14 h-14 bg-white text-black rounded-full flex items-center justify-center transform translate-y-4 group-hover:translate-y-0 transition- duration-75 ease-out ease-out hover:scale-110 shadow-lg"
                      ...
                      {project.github !== '#' && (
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer" 
                          className="w-14 h-14 bg-black/80 border border-white/20 text-white rounded-full flex items-center justify-center transform translate-y-4 group-hover:translate-y-0 transition- duration-75 ease-out ease-out hover:scale-110 shadow-lg"
                  ...
                  <div className="p-6 md:p-8 flex flex-col flex-grow">
                    <h4 className="text-xl font-bold text-white mb-2 group-hover:text-cyan transition-colors duration-75 ease-out">{project.name}</h4>
